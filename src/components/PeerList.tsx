...
export function PeerList({ peers, localPeerName }: PeerListProps) {
    // ✅ FIXED: Remove duplicate peers by ID
    const uniquePeers = Array.from(
        new Map(peers.map(peer => [peer.id, peer])).values()
    );
    ...
    return (
        <div className="bg-white rounded-lg shadow-md p-4">
          ...
          {uniquePeers.length + 1}
            ...
            {/* ✅ FIXED: Make scrollable independently */}
            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                <div className="flex items-center gap-3 p-3 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="font-medium text-gray-900 truncate">{localPeerName}</p>
              ...
              <span className="text-white font-semibold">
                {peer.name.charAt(0).toUpperCase()}
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="font-medium text-gray-900 truncate">{peer.name}</p>
